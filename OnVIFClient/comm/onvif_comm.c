...
int resetInformation(char* des, char* src, int bufferSize)
{
    int i = 0;
    while (i < bufferSize)
    {
    ...
    return addinformation(des, src, bufferSize);
}

int addinformation(char* des, char* src, int bufferSize)
{
    if (des[bufferSize - strlen(src) - 1] == '\0')
    {
        strcat(des, src);
        return 0;
    }
...
int soap_perror_info(struct soap* soap, const char* str, char* dest, int bufferSize)
{
    if (NULL == str) 
    {
        if (
            0 != addinformation(dest, "[soap] error: %d, %s, %s\n", bufferSize) ||
            0 != addinformation(dest, soap->error, bufferSize) ||
            0 != addinformation(dest, *soap_faultcode(soap), bufferSize) ||
            0 != addinformation(dest, *soap_faultstring(soap), bufferSize)
            )
    ...
    {
        if (
            0 != addinformation(dest, "[soap] %s error: %d, %s, %s\n", bufferSize) ||
            0 != addinformation(dest, str, bufferSize) ||
            0 != addinformation(dest, soap->error, bufferSize) ||
            0 != addinformation(dest, *soap_faultcode(soap), bufferSize) ||
            0 != addinformation(dest, *soap_faultstring(soap), bufferSize)
            )
...
void ONVIF_DetectDevice(char* dest, int bufferSize) //void (*cb)(char *DeviceXAddr), char* dest, int bufferSize
{
    int i;
    int result = 0;
    ...
    while (SOAP_OK == result)                                                   // 开始循环接收设备发送过来的消息
    {
        ...
        if (SOAP_OK == result) {
            if (soap->error) {
                soap_perror(soap, "ProbeMatches");                              // soap_perror(soap, "ProbeMatches");
            } else {                                                            // 成功接收到设备的应答消息
                ...
                if (NULL != rep.wsdd__ProbeMatches) {
                    ...
                    for(i = 0; i < rep.wsdd__ProbeMatches->__sizeProbeMatch; i++) {
                        ...
                        //}
                        addinformation(dest, probeMatch->XAddrs, bufferSize);
                        addinformation(dest, "\n", bufferSize);
                    }
                }
            }
    ...
    addinformation(dest, "$", bufferSize); // IP地址结尾
    SOAP_DBGLOG("\ndetect end! It has detected %d devices!\n", count);
    addinformation(dest, "detect end! It has detected ", bufferSize);
    char countNum[20] = {0};
    itoa(count, countNum, 10);
    addinformation(dest, countNum, bufferSize);
    addinformation(dest, " devices!", bufferSize);
    addinformation(dest, "$", bufferSize);
