...
void CMyVideoArea::OnPaint()
{
	...
	ScreenToClient(&m_ScreenRect); // 将屏幕坐标转换为用户坐标
	CPaintDC pDC(this);
	CDC memDc;
	...
	if (memDc)
	{
		...
		memDc.FillSolidRect(&m_ScreenRect, RGB(0, 0, 0)); // 黑色填充背景
...
#pragma region InitalizeRegion
	...
	// char filepath[] = "rtsp://192.168.1.11:554/cam/realmonitor?channel=1&subtype=0&unicast=true&proto=Onvif";
...
void CMyVideoArea::SetFile(CStringA  str)
{
	if (!m_cFileName.IsEmpty())
		m_cFileName.Empty();
	m_cFileName = str; // rtsp://admin:adminadmin123@192.168.3.168:554/onvif1
	// m_cFileName = "http://admin:admin@192.168.1.11/onvifsnapshot/media_service/snapshot?channel=1&subtype=0";
}
...
void CMyVideoArea::PlayBegin()
{
	...
	m_pThreadPlay = AfxBeginThread(ThreadPlay, this); // 解码线程，将文件中的图像放入队列中
	m_pThreadRender = AfxBeginThread(ThreadRender, this); // 渲染线程，将队列中的图像显示出来
}
